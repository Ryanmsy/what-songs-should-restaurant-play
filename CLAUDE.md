...
- `tracks_features_new.csv` (the 114K-track dataset from the cuisine/popularity migration, notebook `01b`): `track_genre` labels are unreliable — assigned by an automated per-genre scrape, never human-verified, so the same artist can appear under contradictory genres (e.g. Michael Bublé under both `country` and `j-pop`; Zeki Müren, a Turkish singer, under `j-rock`; Badfinger under `blues`, `power-pop`, *and* `j-rock`). Checked before trusting a `track_genre` value for anything: `j-rock` and `j-dance` were bad enough (Turkish/Indonesian/British acts in `j-rock`; mostly Jamaican dancehall in `j-dance`) that they were dropped from `CUISINE_GENRE_FILTERS["Cuisine.Japanese"/"Cuisine.SushiBars"]` in notebook `05d` (now `["j-pop", "j-idol", "anime"]`, was `["j-pop", "j-rock", "j-idol", "j-dance", "anime"]`). `j-idol`/`anime` sampled clean; `j-pop` on its own was still ~14%+ confirmed-contaminated (Michael Bublé, Norah Jones, several Turkish/Swedish/Brazilian artists) and, after a live example surfaced 3 non-Japanese tracks in one restaurant's top 5, was further restricted to only the subset with Japanese script (hiragana/katakana/kanji) in the track/artist name — cuts 594 `j-pop` tracks down to 268 verified, the rest relabeled `j-pop-unverified` and no longer matched by the cuisine filter. Real romanized-name J-pop artists with no script anywhere in their catalog entry are a known false-negative cost of this approach. Both the artifact-level `CUISINE_GENRE_FILTERS` fix and the script filter are implemented in notebook `05d` and patched directly into the live `recommender_artifact_v2.pkl` (no full notebook re-run required). The other 7 `CUISINE_GENRE_FILTERS` buckets (Southern, SteakhouseBBQ, TexMex, Mexican, LatinAmerican, Chinese, Indian) were audited the same way (cross-tagging artists against other genres in the raw catalog) and came back clean — the apparent 13–31% flag rates there were legitimate genre overlap (e.g. reggaeton artists also tagged `reggae`/`dancehall`, Brazilian gospel artists also tagged `brazil`), not mislabeling, so they were left unchanged.

## Deferred — Revisit After MVP is Built
- ~~`deployment_draft/artifact/requirements.txt` duplicates `pyproject.toml`'s dependency list~~ — resolved: `pyproject.toml` stays the full dev/notebook environment (adds `matplotlib`/`seaborn`/Jupyter); each `deployment_draft/*/requirements.txt` (artifact, api, streamlit) stays independent and scoped to exactly what that service imports, for lean Docker builds. `deployment_draft/api/requirements.txt` was actually missing `pandas` and `python-dotenv` (both imported by `api_v2.py`) — added. Note for whoever writes the API Dockerfile: `api_v2.py` also imports `scripts/feedback_store.py` via a relative `sys.path` hack two directories up, so that file needs to be `COPY`'d into the image with the same relative layout.
